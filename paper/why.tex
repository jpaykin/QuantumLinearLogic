...
\footnotetext{In this paper, we take a \term{many-worlds} view of quantum
  mechanics; other views are possible, but we only have space to discuss this
  briefly, in \crefme{}.  In particular, the Bohmian pilot-wave interpretation
  of quantum mechanics does have definite states without measurement, but is
  inherently \emph{non-local}, which violates relativity.  \citeme{}}
...
\term{quantum computation}: the design of computation models that take advantage
of the way the world \emph{really} works.  \citeme{so many}
...
result in quantum mechanics, due to \citeme{Wootters and Zurek} and
\citeme{Dieks} (independently), that says there is no operation which can
...
reminiscent of linear logic.  The model we discuss is due to \citeme{that
Abramsky and Coeke paper, of course}; we offer an alternative presentation of
the same ideas.
...
Newtonian.  \Cref{sec:how} will relate these things to the categorical model,
