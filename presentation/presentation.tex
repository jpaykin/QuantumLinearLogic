...
\begin{frame}{Classical computation}
  ...
  (We're still intuitionistic!  Classical is the opposite of quantum.)
\end{frame}
...
\begin{tikzpicture}[every node/.style={draw, circle,
  ...
  \path (0,-\sep) node {Quantum logic}            ++(\sep,0)
                  node {$⊗ \quad 1$\\$⊕ \quad 0$} ++(\sep,0)
                  node {Hilbert spaces}           ;
\end{tikzpicture}
\end{center}
\end{frame}

\begin{frame}{Tensor product, in QM}
  $\text{One qubit: } \qubit ≔ ℂ² / \,{∼}$
  \bigskip
  
  Qubits are linear combinations over the basis:
  \vspace{-\baselineskip}%
  \begin{itemize}
    \item One qubit: $\{\ket{0}, \ket{1}\}$ (in $\qubit$)
    \item Two qubits: $\{\ket{00}, \ket{01}, \ket{10}, \ket{11}\}$ (in $\qubit ⊗
          \qubit$)
    \item …
  \end{itemize}
  \bigskip
  ...
  A two-qubit state is \term{separable} if it has the form $(α₀\ket{0} +
  α₁\ket{1}) ⊗ (β₀\ket{0} + β₁\ket{1})$; otherwise, it is \term{entangled}.
\end{frame}
...
\begin{center}
A \term{scalar} is a morphism $1 → 1$\par

\vspace{3\bigskipamount}

For $f : A → B$ and $s : 1 → 1$, $s • f : A ≃ A⊗1 → B⊗1 ≃ B$
\end{center}
\end{frame}
...
\begin{column}{0.5\textwidth}
  Consider\\$\quad\ β_{i,j} : \qubit ⊗ \qubit^* → 1$\\where, for instance,
  \begin{align*}
    β_{0,0}(\ket{00}) &= 1 \\
    β_{0,0}(\ket{01}) &= 0 \\
    β_{0,0}(\ket{10}) &= 0 \\
...
\begin{frame}{$⊕$, categorically}
  ...
  \[ β_{0,0},β_{0,1},β_{1,0},β_{1,1} : Q ⊗ Q^* → 1 \]

  \bigskip\bigskip
  
  We make $⊕$ be a biproduct, so we have
  \[ ⟨β_{0,0},β_{0,1},β_{1,0},β_{1,1}⟩ : Q ⊗ Q^* → 1⊕1⊕1⊕1 \]
\end{frame}
